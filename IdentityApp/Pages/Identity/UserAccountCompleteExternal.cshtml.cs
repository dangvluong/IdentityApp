...
namespace IdentityApp.Pages.Identity
{
    ...
    public class UserAccountCompleteExternalModel : UserPageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string provider)
        {
            ...
            bool valid = await UserManager.VerifyUserTokenAsync(IdentityUser, UserManager.Options.Tokens.PasswordResetTokenProvider, UserManager<IdentityUser>.ResetPasswordTokenPurpose, decodedToken);
            if (!valid)
            {
        ...
        public async Task<IActionResult> OnGetCallbackAsync()
        {
            ...
            IdentityResult result = await UserManager.AddLoginAsync(IdentityUser, info);
            if (!result.Succeeded)
            {
        ...
        {
            if ((id == null
            || (IdentityUser = await UserManager.FindByIdAsync(id)) == null) && !TempData.ContainsKey("errorMessage"))
            {
